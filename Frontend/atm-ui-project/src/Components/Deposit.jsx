...
import {makeDeposit} from '../util/functionality.js';
function Deposit({token,accounts}){
    const [sourceAccount,setSourceAccount] = useState(null);
    const [amount,setAmount] = useState(0);
    const navigate = useNavigate();
    
    async function handleDeposit(){
        await makeDeposit(sourceAccount,amount,token);

        setAmount(0);
        setSourceAccount(0);
    }

    const hasSelectedValidOptions = amount > 0 && sourceAccount > 0;
    ...
    return (
        <div className='deposits'>
          ...
          <button className='deposit-button' onClick={handleDeposit} disabled={!hasSelectedValidOptions}>Deposit</button>
